...
struct Cmd {
    ...
    /// Do not include the "generated by" comment in output
    #[clap(long)]
    no_generated_by: Option<bool>,
}
...
fn main() -> Result<()> {
    ...
    if let Some(true) = no_generated_by {
        file_contents += format!(
            "/// The WIT below was automatically generated by webidl2wit v{}\n",
            env!("CARGO_PKG_VERSION")
        )
        .as_str();
    }
    file_contents.push_str(pkg.to_string().as_str());

    if let Some(ref output_path) = wit_path {
        std::fs::write(output_path, &file_contents).with_context(|| {
